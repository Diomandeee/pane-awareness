...
class VaultWriter:
    ...
    def write_pane_state(self, pane: dict) -> Path:
        ...
        tty_short = self._safe_filename(
            tty.split("/")[-1] if "/" in tty else tty
        )
    ...
    def write_handoff_thread(self, handoff: dict) -> Path:
        ...
        safe_from = self._safe_filename(from_label)[:15]
        safe_to = self._safe_filename(to_label)[:15]
        filename = f"{ts}-{safe_from}-to-{safe_to}.md"
    ...
    def update_note_section(
        ...
        *,
        ...
        if not str(resolved).startswith(str(vault_resolved) + os.sep):
            raise ValueError(f"Path escapes vault root: {resolved}")
